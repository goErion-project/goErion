...
class PGP
{
     ...
     * @throws \Exception
     */
    public static function EncryptMessage($message,$key): string
    {
        if ($message == null)
        {
            throw new \Exception("Please specify a message to encrypt",1);
        }
        if ($key == null)
        {
            throw new \Exception("Please specify a key to encrypt with",1);
        }
        $pubkey = $key;
