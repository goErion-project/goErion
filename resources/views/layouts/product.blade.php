                                    ...
                                    <td>
                                        <ul>
                                            @foreach($product -> offers as $offer)
                                                <li>
                                                    <strong>@include('includes.currency', ['usdValue' => $offer -> dollars])</strong> per {{ \Illuminate\Support\Str::plural($product -> mesure, 1) }},
                            ...
                            <tr>
                                ...
                                <td>
                                    @foreach($product -> getCoins() as $coin)
                                        <span class="badge badge-indigo">{{ strtoupper(\App\Models\Purchase::coinDisplayName($coin)) }}</span>
                                    @endforeach
                                </td>
                            </tr>
                            <tr>
                                @if(!$product -> isUnlimited())
                                    ...
                                    <td>
                                        ...
                                        <span class="badge badge-light">{{ $product -> orders }} {{ \Illuminate\Support\Str::plural($product -> mesure, $product -> orders) }} </span>
                                    </td>
                                @endif
                            </tr>
                            <tr>
                                ...
                                <td>
                                    @if(count($product -> getCoins()) > 1)
                                        <select name="coin" id="coin" class="form-control form-control-sm">
                                            @foreach($product -> getCoins() as $coin)
                                                <option value="{{ $coin }}">{{ strtoupper(\App\Models\Purchase::coinDisplayName($coin)) }}</option>
                                            @endforeach
                                        </select>
                                    @elseif(count($product -> getCoins()) == 1)
                                        <span class="badge badge-mblue">{{ strtoupper(\App\Models\Purchase::coinDisplayName($product -> getCoins()[0])) }}</span>
                                    ...
                                    @if(count($product -> getTypes()) > 1)
                                        <select name="type" id="type" class="form-control form-control-sm">
                                            @foreach($product -> getTypes() as $type)
                                                <option value="{{ $type }}">{{ \App\Models\Purchase::$types[$type] }}</option>
                                            @endforeach
                                        </select>
                                    @elseif(count($product -> getTypes()) == 1)
                                        <span class="badge badge-mblue">{{ \App\Models\Purchase::$types[$product -> getTypes()[0]] }}</span>
